...
/*
 * Acorn's cache on the same Redis WordPress uses.
 *
 * Two caches, two sets of variables. The block above configures the object cache drop-in from
 * WP_REDIS_*; Acorn's Cache facade reads CACHE_STORE, REDIS_HOST and REDIS_PASSWORD, which nothing
 * sets. Unset, Acorn falls back to the `file` driver, and on ECS that means every task caches on
 * its own filesystem: the hourly job warms the marketing snapshot on whichever task runs cron, and
 * a dashboard request served by any other task shows what that task last computed. On 2026-09-20
 * the Slack card read correctly at 07:00 beside a widget still showing 06:56, same site, same
 * moment.
 *
 * Derived rather than configured, so no task definition or secret has to change. An environment
 * with no Redis is untouched and keeps the file driver.
 *
 * ## Why here, and why it took three attempts
 *
 * Deriving this in docker/entrypoint.sh applied it everywhere before anything had been proven,
 * which is how it reached production twice in one morning — the first time because
 * illuminate/redis was not installed, so `redis` resolved to phpredis' own class and RedisStore
 * refused it with a TypeError on every admin screen. A GitHub secret would work but lives outside
 * the repo, where nobody reading this file would find it.
 *
 * It then ran on staging alone for a while, because Acorn's cache has no graceful mode and
 * LiveCallAvailabilityRouter, CalendlyTokenPool, CalendlyClient and CalendlyMetadataCache read
 * through it on the public booking path: an outage would have gone from "wrong dashboard" to
 * "nobody can book". Those callers now go through SoftCache, which degrades every operation to the
 * empty-cache answer they already handle on a cold container, so the asymmetry has no reason left.
 *
 * Requires illuminate/redis and the provider ThemeServiceProvider registers. See .env.example.
 */
