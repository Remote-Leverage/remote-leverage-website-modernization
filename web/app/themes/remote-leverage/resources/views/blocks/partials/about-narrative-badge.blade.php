{{-- Dark purple pill badge with brand swirl icons. Always ACF-driven ($badgeText), in both
     `acf` and `inner_blocks` modes of AboutNarrativeBlock — extracted so the two layout branches
     in about-narrative.blade.php don't duplicate this markup. --}}
<div class="inline-flex items-center gap-3 bg-[#250D4A] rounded-full px-5 py-3 shadow-[0_4px_20px_rgba(37,13,74,0.2)]">
    {{-- Swirl Icon Left: the real brand swirl mark (resources/images/pages/about-us/remote-leverage-swirl-icon.png),
         replacing the earlier CSS-gradient circle placeholder. --}}
    <div class="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden shrink-0">
        <img src="{{ $badgeSwirlIcon }}" alt="" width="32" height="32" loading="lazy" decoding="async" class="w-full h-full object-cover">
    </div>
