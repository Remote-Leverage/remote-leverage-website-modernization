...
| Class | Does |
| :--- | :--- |
| `AttributionCollector` | Every click parameter, UTM and identifier on the request, read **URL first, then the site's own `rl_*` cookies, then HandL's leftovers**. The `rl_*` cookies are written on every page by `TrackingHooks::injectAttributionCookies()` — the job the HandL UTM Grabber did on the old site, which did not come across at cutover, so between 2026-09-19 and 2026-09-26 attribution only came from the URL of the page a visitor booked on. Last touch (`rl_<param>`, 30 days) is replaced as a set by any visit carrying a click parameter; first touch (`rl_ft_<utm>`), landing page and original referrer (90 days) are written once. `rl_fbclid_ts` keeps the click time, so an `fbc` built from a remembered `fbclid` says when the click happened. HandL's `fbclid` cookie is never read: it is frozen, has no click time, and attached a 28 August click to 25 September bookings. |
...
| `FbpResolver` | The same forced-live-update treatment for `_fbp`, used only by `CaptureLeadAction`. Simpler than `FbcResolver`: `_fbp` is not tied to a click, so there is no click-match check and no synthetic value to upgrade from — a live cookie just wins over whatever `attribution.handl._fbp` this request or an earlier one carried, and the resolver stays silent for callers with no attribution of their own. Writes into the blob, not a column — `rl_leads` has none for `_fbp`. |
