...
| Session isolation behind CloudFront | [known-issues.md](known-issues.md) #23 | 🟡 **New gate, 2026-09-17 — closed in code, not yet closed in front of a visitor.** Four routes (`/book-consultation`, `/referral-dashboard`, `/partners`, `/social-media-kit`) sent `Set-Cookie: laravel-session=…` together with `public, s-maxage=60`; CloudFront cached the cookie and handed every visitor in the TTL window the same session — which, because the referrer portal restores a login from `session('referrer_code')` with no password re-check, is the same *login*. Reproduced on **both** `staging` and `production.remoteleverage.com`. `docker/nginx.conf` now keys cacheability on the response (`$upstream_http_set_cookie`) instead of a path list. **Closing it needs a deploy *and* a CloudFront invalidation on both distributions** — entries already cached keep serving the shared cookie until they age out |
...
1. **Deploy the `docker/nginx.conf` session fix and invalidate both CloudFront distributions.**
   First because it is live on both hosts right now: everyone browsing shares one
   `laravel-session`, and therefore one referrer-portal login. The invalidation is not optional —
   the fix does not evict what is already cached. [known-issues.md](known-issues.md) #23,
   [deployment.md](deployment.md).
2. Set the webhook signing secrets; repoint the four Calendly redirects. Cheap, and they
   silently break paying customers. (`STRIPE_DEFAULT_THANKYOU_URL` no longer needs a value —
   closed 2026-09-15, see below.)
3. Upload `5-minute-VSL_Horizontal_V01.mp4` to `uploads/videos/` on EFS in staging **and**
   production, and in the same pass repoint `/about-us/` at its pattern on both — the page holds
   expanded markup with the old video URL in `post_content`, so the upload alone fixes nothing
   there. Neither half is something a deploy can do.
   [known-issues.md](known-issues.md) #24, [deployment.md](deployment.md) § Content import.
4. Run `wp acorn content:rehost-seo-images` against each environment's database, and re-pick
   `company_logo` by hand in wp-admin Search Appearance — the command reports that field and
   deliberately does not write it.
5. Repeat the Yoast configuration and `content:import-seo` on staging and production.
6. Take the performance numbers **on staging** — the local run is not what the gate asks for.
7. Build production infrastructure and the DNS/rollback runbook. **Owned outside this workstream,
   still a hard gate.**
8. Resolve Sentry's DSN and the `MAIL_*` values; close the remaining gates; flip.
...
- [x] **`robots.txt` exists** — **generated per host since 2026-09-17**, by `App\Support\SiteRobotsTxt` on the `robots_txt` filter; the static `web/robots.txt` is deleted. Same directives (`/wp/wp-admin/`, the authenticated portals, `/api/`), but the sitemap line now comes from Yoast at priority 99999 using its own base URL instead of being hardcoded. The static file advertised `https://remoteleverage.com/sitemap_index.xml` on *every* host, so staging and the preview host were sending crawlers to the legacy site's sitemap — a file on disk cannot know which host is serving it. The wp-admin pair comes from core's `do_robots()` — Bedrock-aware through `admin_url()` — rather than being restated, and the portal and `/api/` disallows are appended on every host, the noindex ones included. (Locally it still reports 404 — a Herd/nginx artifact affecting `robots.txt` and `favicon.ico` only. Production serves it 200. See known-issues.md #3.)
...
- [ ] **Re-point the OpenGraph images, and re-pick `company_logo`** — **still open; the fix exists as a command and has been run against nothing.** Verified live 2026-09-17 on staging and on the production preview host: every sampled page except the homepage emits an `og:image` on `https://remoteleverage.com/wp-content/uploads/…`. Harmless while the legacy site answers; the moment it stops, the new site's social cards break — and until then every share advertises the old host.

  Measured the same day against the local database with `wp acorn content:rehost-seo-images --dry-run`: **171** `_yoast_wpseo_opengraph-image` rows on the legacy host, of which **134** match a local attachment by basename and are rewritten and **37 rows across 24 distinct filenames** have no local media — mostly stock (`pexels-*`, `*-unsplash-*`), but including `image-9.png` (the `/hire-va-4/` card) and `stealing-jobs-hero.webp`. The media library was deliberately never ported, which is why the unmatched set exists at all.

  `App\Domains\ContentAudit\Commands\RehostSeoImagesCommand` resolves each URL through `BlockDefaults::mediaUrl()` — the same basename resolver the migrated presets use, so a file re-uploaded under a different year/month still matches — and is idempotent. It defaults to `--source=https://remoteleverage.com` and `--meta-keys=_yoast_wpseo_opengraph-image,_yoast_wpseo_twitter-image`. The one real decision is `--unmatched`: the default `drop` deletes the row so Yoast falls back to the site-wide default image (a picture from *this* site), while `--unmatched=keep` leaves it pointing at the legacy host — use `keep` with `--dry-run` to get the full report, upload what is missing, then re-run. **Run it once per environment**, since it rewrites a database and not a file.

  **`company_logo` is not covered by that run and needs a human.** Yoast's `wpseo_titles['company_logo']` is still `https://remoteleverage.com/wp-content/uploads/2024/12/Logo-Square-1024-x-1024-px.png`. The command reports it (alongside `person_logo` and `og_default_image`) and deliberately refuses to write it: each is half of a URL/attachment-id pair, and rewriting the URL alone leaves Yoast internally inconsistent in a way the admin screen will not show. Re-pick it in **wp-admin → Yoast SEO → Search Appearance** so both halves stay in step.

  *Correction, 2026-09-17:* an earlier pass reported `company_logo` as already fixed. It was not — that check grepped rendered HTML for a schema node which does not appear on a `noindex` host, so its absence proved nothing about the stored value. Read `wpseo_titles`, not the page source.
