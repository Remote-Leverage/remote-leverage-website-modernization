...
3. Upload `5-minute-VSL_Horizontal_V01.mp4` to `uploads/videos/` on EFS in staging **and**
   production. `/about-us/` renders a broken player on both until it lands, and no deploy will
   change that — the file is deliberately not in git or the image.
   [known-issues.md](known-issues.md) #24.
