...
`send_default_pii` stays `false` — that flag attaches IP (and other request data) to *every* event
this site sends to Sentry, including ones with nothing to do with a specific visitor. Instead
`SentryReporting::identifyUser()` sets the Sentry user explicitly, on the user identity only,
choosing one of two identities:
