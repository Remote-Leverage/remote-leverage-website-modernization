...
| `SENTRY_ENVIRONMENT` | **ECS task env var**, same value as `WP_ENV` (`staging` / `production`). Not a secret. Unset, both SDKs fall through to `WP_ENV` and then Laravel `APP_ENV`, then to `production` — which is why staging events used to be unfilterable from prod. |
| `SENTRY_SEND_DEFAULT_PII` | **GitHub Environment secret**, copied into Secrets Manager and mapped on the ECS task definition. `true` on staging and production. Unset, `config/sentry.php` defaults to `false`. |
