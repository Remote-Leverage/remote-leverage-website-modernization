...
| `SENTRY_ENVIRONMENT` | **GitHub Environment secret**, copied into Secrets Manager like the rest. `staging` on staging, `production` on production. Unset, both SDKs fall through to Laravel `APP_ENV` and then to `production`, so staging events are unfilterable from prod. |
| `SENTRY_SEND_DEFAULT_PII` | **GitHub Environment secret**, copied into Secrets Manager and mapped on the ECS task definition. `true` on staging, `false` on production. Unset, `config/sentry.php` defaults to `false` so anonymous visitor IPs are not attached to errors. |
