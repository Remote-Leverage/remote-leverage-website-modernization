...
**ADR-0008** (Lead domain, retire Gravity Forms) is now `Status: Accepted`. The transport question it left open is resolved as **synchronous subscriber** (matches the implementation — no listener uses `ShouldQueue`), consistent with WR-106 being on hold (no queue worker deployed). **Superseded 2026-09-22:** `CallHandlerJob` now implements `ShouldQueue` and the entrypoint provisions a worker; one call site is converted and `QUEUE_CONNECTION` is unset in every deployed environment, so the effective transport is unchanged. See [`known-issues.md`](known-issues.md).
