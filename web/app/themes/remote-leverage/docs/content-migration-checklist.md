...
- [x] Install **Yoast SEO** on v2 (live site runs Yoast SEO Premium v28.4) — matching plugin means `_yoast_wpseo_*` postmeta, schema graph, and canonical URLs carry over directly instead of needing field-mapping into a different plugin's schema. Free 28.5 via `wp-plugin/wordpress-seo`; settings left unconfigured pending the `/guides/` vs `/blog/` decision below
- [ ] Carry the meta across: `wp acorn content:import-seo` (dry run 2026-09-15 — 187 of 355 production items match a local slug, 168 production items have no local counterpart). See [seo-meta-migration.md](seo-meta-migration.md)
