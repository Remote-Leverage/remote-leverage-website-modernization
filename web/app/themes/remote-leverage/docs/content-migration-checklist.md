...
> including the ~45 role/industry SEO templates, the orphaned operational pages, the A/B
> experiment set, and the tools/lead magnets. **§3 (partner hub) is the exception — it was
> confirmed in scope on 2026-09-14 and its items are live work.**
>
> Two directives from the earlier version of this banner still stand, because they moved
> pages *into* scope rather than out:
>
> - Pages on the transfer list are in scope **regardless** of any "confirmed dead" verdict in
>   §4 or "A/B experiment" verdict in §5 — affecting `vastore5`, `hire-va-isolated-form`,
>   `1monthonus`, `1monthonus-flp`, `stealing-jobs`, `stealing-jobs-lp`, `steal-back-your-time`,
>   `hire-va-6`, `hire-va-email`, `hire-va-1st-month-free`, and the three
>   `hire-virtual-assistants-from-latam-...-variant` pages.
> - **`hire-va-4` was added to scope on 2026-09-14 and is now migrated** (page ID 1000000,
>   rendering `patterns/hire-va-4-full.php`). §5 still lists it as an experiment pending a
>   "confirm it's the winner" call; that call is made and the work is done.
>
> **What this document is now:** a read-only inventory of what exists on production, useful
> for writing cutover redirects and for confirming a page really is out of scope before
> dropping it. Nothing more.
>
> **Source of truth for the migration:**
> [PAGE-MIGRATION-STATUS.md](../../../../../PAGE-MIGRATION-STATUS.md) — including §4, which
> inventories v2 content built *before* scope closed and now needs a keep/redirect/delete
> decision.
...
- [x] [`/affiliate-program/`](https://remoteleverage-v2.test/affiliate-program/) and [`/referral/`](https://remoteleverage-v2.test/referral/) — local page IDs 212 and 213. `/affiliate-program/` is a genuine new marketing page (new `AffiliateHeroBlock` + `ProcessStepsBlock` + `BookingFooterBlock`); both its "Apply/Join" CTAs link to the existing `/referrer-register` route (`referrer.register`, `referrer-register.blade.php`) and its "Book a Strategy Call" CTAs scroll to `#booking-footer`. ~~`/referral/` turned out NOT to be a referral-program explainer — production content is section-for-section identical to the already-migrated VA hiring landing page (ID 104, `hire-va-4-preview`), so it was republished with that same proven block content rather than new copy;~~ **❌ This claim is wrong — corrected 2026-09-14.** Production `/referral/` (ID 27582, titled *"Home – Referral"*) is a referral-traffic variant of the **homepage**, not of the VA hiring landing page. Verified by fetching all three production pages and comparing their `h1`–`h3` headings: `/referral/` shares **27 of 27 headings (100%) with the production homepage** and only **4 of 27 (15%) with `/hire-va-4/`**. Its real content is the homepage's — "Great Talent Changes Everything", the talent-profile cards, "World's Best Talent, Hired Directly for You", "Beyond the 'Virtual Assistant.'" — none of which appear on `/hire-va-4/`. The local page (ID 213) was therefore built with the **wrong source content**. `/referral/` is out of scope under scope closure, so this is recorded as a correction rather than rework; see PAGE-MIGRATION-STATUS.md §4a. Referral attribution is handled separately by `ReferralAttributionMiddleware` regardless of page content.
- [x] `/blog/` archive is canonical (confirmed 2026-09-15) — `/guides/` (not in sitemap, see §4) 301s to `/blog/` via `config/redirects.php`, not Yoast's redirect manager: Premium is not being bought
...
## 2. Role/industry landing pages ("Virtual Assistants" SEO templates) — ⛔ OUT OF SCOPE
...
## 3. Partner hub — ✅ IN SCOPE (confirmed 2026-09-14)

The hub is kept **alongside** the standalone `/remote-leverage-x-*/` landing pages — they do
different jobs and neither folds into the other. `/partners/` is the one addition to the
transfer list, since it is v2-native and has no production counterpart.

Lexgo rides the hub: production has no `/remote-leverage-x-lexgo/` page, so the hub entry is
its only home. Three entries needed — `oyster` (exists as a 66-char stub), `lano`, `lexgo`.
See [PAGE-MIGRATION-STATUS.md](../../../../../PAGE-MIGRATION-STATUS.md) §3 P1.
...
## 4. Pages excluded from the sitemap (noindex / orphaned) — ⛔ OUT OF SCOPE except transfer-list rows
...
## 5. In-sitemap A/B-test / experiment pages — ⛔ OUT OF SCOPE except transfer-list rows
...
## 7. Tools / lead magnets — ⛔ OUT OF SCOPE
...
- [ ] Real Estate Posts — 9
- [ ] Salary Guides — 12
- [x] Live Sessions — 2 — **not a gap (resolved 2026-09-15).** Production registers `category` on the `page` post type, and both Live Sessions items are `post_type=page`, not posts: 41629 `/live-session-build-ai-tools-for-businesses/` and 40118 `/live-session-10x-revenue-with-ai/`. Both are in Yoast's `page-sitemap.xml` and absent from `post-sitemap.xml`. The "2 on production, 0 locally" discrepancy was a term-relationship count compared across mixed post types — the term count is not post-type-scoped. Nothing was dropped. As pages they fall in the discarded 191 and need a redirect, not an import; both are expired registration pages ("Registration has now closed for this live session"). Note the archive URL is `/blog/category/live-sessions/` — `/category/live-sessions/` 404s.
- [ ] News — 2
...
Done locally on 2026-09-15. The numbers and the evidence are in
[seo-meta-migration.md](seo-meta-migration.md); this is the checklist view.

- [x] Install **Yoast SEO** on v2 (live site runs Yoast SEO Premium v28.4) — matching plugin means `_yoast_wpseo_*` postmeta, schema graph, and canonical URLs carry over directly instead of needing field-mapping into a different plugin's schema. Free 28.5 via `wp-plugin/wordpress-seo`, **activated 2026-09-15**
- [x] **Carry the meta across** — `wp acorn content:import-seo` run for real 2026-09-15: **190 local items now carry production's meta, 863 postmeta rows, from 0**. 355 production items fetched, 165 with no local counterpart (the §2 role/industry templates and the orphaned operational pages, both out of scope), 0 local items without a production counterpart. 23 case studies matched across post types, since production serves its case studies as pages
- [x] **Canonicals rewritten onto the local host** — all 180 canonical rows resolve to `remoteleverage-v2.test`; **0** still contain `remoteleverage.com`. Carrying them verbatim would have pointed every v2 page at production
- [x] **`referral-program` and `ecommerce-virtual-assistant` excluded from production's `noindex`** — both were rebuilt in v2, so the suppression is stale. Implemented as `--force-index=`, an option on the importer defaulted to those two slugs, so the exclusion survives a re-run instead of being a manual delete that the next run undoes. The other five (`signedup`, `vaonboardingform`, `payment`, `vastore5`, `referral-program-thank-you-page-deposit`) inherit it, verified page by page
- [x] **Re-point the `post` archive canonical** — `/blog/` wins (see §1). Production already does this; `/guides/` 301s onto `/blog/` on both sites, there is no local `guides` page, and page 799 carries `_yoast_wpseo_canonical = https://remoteleverage-v2.test/blog/`. No `/guides/` URL appears in the sitemap
- [x] **Carry over Organization/WebPage schema graph settings** — site representation, social profiles and org details set from production's rendered `Organization` node via `wp option patch update`, not through Yoast's first-time wizard, and the wizard marked finished so it cannot overwrite them later
- [x] **The sitemap question is settled** — Yoast's `/sitemap_index.xml`, with core's `/wp-sitemap.xml` 301ing onto it, matching production. `web/robots.txt` re-pointed
- [x] ~~Set up Yoast's redirect manager (Premium) for every URL in §4/§5 that gets killed rather than rebuilt~~ — **not doing it.** Yoast Premium is not being bought (decided 2026-09-15). Redirects stay in `config/redirects.php`, which the cutover gate already records as complete at 166 entries
- [ ] **Rule on `services`, `store` and `contractoragreement`** — created as v2 pages *after* the exclusion decision, while this import was being verified, and all three inherited production's `noindex`. The cutover doc describes them as real v2 rebuilds, which is the same reasoning that excluded the two pages above. Add them to `--force-index` if so
- [ ] **Re-point `company_logo` and the 171 `opengraph-image` URLs** off `remoteleverage.com` once the media library lands
- [ ] **Repeat both steps on staging and production** — activate and configure Yoast, then re-run the import. It is idempotent: a second run reports 0 changes
